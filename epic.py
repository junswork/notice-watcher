...
처음에 로그인이 안 됐던 이유는 3단계 다음에 한 걸음이 더 있었기 때문이다.
포털은 로그인 결과를 **자동 제출 폼**이 담긴 페이지로 돌려준다. 브라우저는
그 폼을 자바스크립트로 곧바로 다시 보내는데, 우리는 자바스크립트를 돌리지
않으니 거기서 멈춰 세션이 안 붙었다. 그 폼을 대신 보내 주니 통과했다
(`follow_auto_submit`).
